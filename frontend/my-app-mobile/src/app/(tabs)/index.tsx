...
export default function Index() {
    ...
    useEffect(() => {
        ...
        const connectASR = async () => {
            ...
            ws.current.onerror = (e) => {
                console.log("ASR socket error : ", e.target);
            };
    ...
    const handleStopRecording = async () => {
        ...
        if (!base64string) {
            showAlert('Error', 'Could not load audio');
            return;
        }
        ...
        if (nlpRunning.current) {
            showAlert('Error', 'NLP is already running');
            return;
        }
        ...
        <SafeAreaProvider>
            <SafeAreaView className='flex-1'>
                ...
                <View className="flex-1">
                    <WebView
                        source={{
                            uri: `${constants.PIXEL_STREAMING_URL}/player.html?ss=${constants.SIGNALING_SERVER}`
                        }}
                        style={{ flex: 1 }}
                ...
                <View className="flex-row items-center p-3 gap-3 bg-white">
                    <TextInput
                        className="grow h-12 border-2 border-gray-600 rounded-md"
                        editable
