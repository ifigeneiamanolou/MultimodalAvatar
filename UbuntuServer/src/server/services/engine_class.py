...
class OrpheusModel:
    ...
    def generate_tokens_sync(self, prompt, voice=None, request_id="req-001", temperature=0.6, top_p=0.8, max_tokens=1200, stop_token_ids = [49158], repetition_penalty=1.3):
        ...
        async def async_producer():
            ...
            try:
                async for result in self.engine.generate(prompt=prompt_string, sampling_params=sampling_params, request_id=request_id):
                    ...
                    if (producer_index % 100 == 0):
                        logger.info(f"placed item {producer_index} in the queue waiting to be dispatched to the decoder")
                    producer_index += 1
        ...
        while True:
            ...
            if token is None:
                break
            if (consumer_index % 100 == 0):
                logger.info(f"consuming token of length {len(token)} of id {consumer_index}")
            consumer_index += 1
            yield token
